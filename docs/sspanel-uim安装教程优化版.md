...
> ℹ️ 本教程已实际验证，解决了 inet4/uuid 类型错误、数据库连接失败、PHP 版本错误、Composer 版本过旧、Nginx 用户权限等所有常见坑。域名示例：tlte.top
...
> ⚠️ Nginx 官方源安装后默认以 `nginx` 用户运行，而 PHP-FPM 和文件所有者是 `www-data`，不一致会导致 502 错误。必须在配置站点前就改掉。
...
# 关键：所有包名必须指定 8.4
apt install -y php8.4-{bcmath,bz2,cli,common,curl,fpm,gd,gmp,\
  intl,mbstring,mysql,opcache,readline,redis,soap,xml,yaml,zip}
...
> ℹ️ 用 `tee` 写配置文件，避免直接粘贴时内容截断。将 `tlte.top` 替换为你的域名。
...
- 定期备份数据库：`mysqldump` 或 `mariadb-dump`
- 及时更新 SSPanel：`git pull` + `composer install --no-dev`
...
- 将 Nginx 用户修改提前到第二步安装后立即执行，避免后续出现 502 困惑
- 修正了管道符号 `|` 在原文档中显示为全角 `│` 的问题，确保可直接复制粘贴
- 第七步去掉了冗余的 `mkdir -p`，避免目录已存在时 git clone 失败
- 所有步骤可直接复制到终端执行，无需额外修改（除密码和域名外）
